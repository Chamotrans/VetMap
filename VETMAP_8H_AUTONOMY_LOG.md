...
### Release recheck — 2026-08-09

- Trigger: Rechecked the Apple Developer Relations `ITMS-90382 Upload limit reached` delivery failure for Version 1.0 Build 39 after the required one-day wait.
- Upload-window result: The conservative retry threshold of 2026-08-04 03:00 CST has passed. The quota wait is complete; this does not itself authorize or prove a new upload.
- Repository evidence: `HEAD` and `origin/main` both resolved to `530313a02b4bc2a2f0b521455fd4d674e9a83512`. GitHub `Backend and Config Validation` run `30764998963` completed successfully for that exact SHA, including the Firebase rules/functions job.
- ASC GET-only evidence: iOS 1.0 version `583c6199-bf1e-42d9-8bd4-69c2e51f4d2c` and review submission `6e901867-779d-454f-8258-bf434993744c` both remained `READY_FOR_REVIEW`; `submittedDate` remained null. Build 12 (`e1cd2911-c0c2-4cd7-94c4-985c2295794a`) remained attached and `VALID`; the latest processed build remained 35, so Build 39 still had not entered processing.
- Xcode Cloud evidence: `Default` workflow `DBB6C988-A379-476E-9E99-3235B11BAD2E` remained disabled and editable, last modified `2026-08-02T18:42:54.261Z`. The latest run remained 41 (`fe0b7c5b-da2a-4199-8576-db3f6ff04153`, `SUCCEEDED`); no Run 42 was created.
- Device evidence: Physical iPhone `是條小狗` (`56F6A8C2-8B84-5328-B167-3567CBBDCE54`, iPhone 16) was paired and available. No install, launch, screenshot, or functional smoke test was performed or inferred.
- Browser boundary: Both available ASC browser sessions had expired to Apple sign-in, so no UI mutation was attempted and no password or 2FA handling was performed. The narrow live state above came from authenticated GET-only ASC API reads.
- Decision: No Cloud workflow activation or run was performed without a fresh explicit account-holder instruction after the quota reset. Content Rights also remains pending the complete attestation covering the clinic database, Hong Kong service directory, official insurance links, and user-submitted content under the app terms.
- Action boundary: No local `xcodebuild`, Apple upload, ASC mutation, build attachment, App Review submission, Firebase deployment, production write, public release, install, launch, or other device action occurred. Registration, submissions, reporting, blocking, chat and all community interactions remain open. All user-owned dirty and untracked files remained untouched.
- Next: After the account holder provides the complete Content Rights attestation and explicitly says to continue, trigger one new Xcode Cloud candidate, wait for ASC processing, install it on `是條小狗`, then deploy the compatible least-privilege rules/function and execute the full chat/account-deletion smoke test before final ASC reconciliation.

### App Review notes drift gate — 2026-08-09

- Finding: The live ASC metadata updater already described how a reviewer opens private chat, reports or deletes a message, blocks the other participant, and reaches account deletion, but the repository's manual `AppStoreSetup.md` Review Notes had retained the older pre-chat five-step path. A later manual paste could therefore remove the reviewer-accessible chat instructions even though the app and live payload supported them.
- Implementation: Updated the manual Review Notes to retain `Send Message`, the Messages tab fixture, incoming-message report, conversation block, own-message deletion, account deletion, two-participant privacy, reported-message moderation, and the VetMap-owned chat fixture declaration.
- Fail-closed regression: Added `scripts/validate_app_review_notes.test.mjs`. It extracts both the fenced manual notes and the ASC updater payload and requires the same eleven 1.0 review facts in each. GitHub `Backend and Config Validation` now runs this test before Functions/rules validation.
- Local evidence: Node test 1/1 passed and `git diff --check` passed. This is documentation/payload consistency proof, not an iOS build or a live ASC mutation.
- Preserved scope: No local `xcodebuild`, Xcode Cloud run, Apple upload, ASC change, Firebase deployment, production write, App Review submission, public release, install, launch, or device action occurred. Registration, submissions, reviews, quotes, reports, blocks and chat remain open; user-owned dirty/untracked files were not modified or staged.

### Completion audit and blocked handoff — 2026-08-09 18:34 CST

- Requirement audit: The six unchecked entries in `PREFLIGHT_CHECKLIST.md` are (1) install a compatible atomic-marker candidate before deploying the least-privilege chat rules/function, (2) obtain the account holder's complete Content Rights attestation and explicit continue instruction, (3) produce and process the new Xcode Cloud candidate, (4) complete the full TestFlight device smoke test, (5) perform the final ASC page-by-page reconciliation, and (6) stop before formal submission for the account holder. They form one ordered external dependency chain; none can be truthfully accepted from source or CI evidence alone.
- Repository and CI: `HEAD` and `origin/main` both resolved to `99f01e3b9efe738d0cba66ec462e7adb689dfe57`. GitHub `Backend and Config Validation` run `31308537417` completed successfully for that exact SHA, including the App Review notes drift gate, Functions validation, emulator-backed account purge, Firestore／Storage rules, catalog gates and patch hygiene.
- Production public audit: The read-only public-only audit passed for the currently deployed state: 180 approved visible clinics (179 authorised catalog entries plus one VetMap demo), 161 mappable and 18 list-only entries, 11 availability overlays (10 24-hour and one scheduled), one demo review, one demo quote, 124 HK services and three official insurance links. Anonymous access to the isolated legacy catalogs remained denied. This public-only mode cannot prove hidden/stray inventory and is not represented as an authenticated full audit.
- ASC GET-only evidence: iOS 1.0 and review submission remained `READY_FOR_REVIEW`, with `submittedDate: null`; Build 12 remained attached, `VALID` and unexpired. The newest processed build remained 35. Workflow `Default` remained disabled and editable; after reading all 39 returned runs and sorting locally, the latest remained Run 41 (`SUCCEEDED`). No Run 42 or new processed candidate existed.
- Device evidence: Physical iPhone `是條小狗` remained paired and available. No candidate install, launch or functional smoke test was performed or inferred.
...
### Release continuation evidence — 2026-08-12

- Source and CI: Feature commit `98809ad` passed GitHub run `31413231064`. Its Xcode Cloud Run 42 `b5c3bdf5-bd8d-4063-a29e-89e5c4a05a06` was from that source but failed Archive at `ProfileTab.swift:366` because of the `Section` overload; it produced no build resource and must not be retried or represented as a candidate.
- Correction and source acceptance: Commit `bb9f484` (`fix: disambiguate favourites section header`) passed GitHub run `31576278579`; local Node regressions passed `91/91`, Swift parse passed, and the fresh Sol verdict was ship.
- Cloud and ASC: Exact `bb9f484` produced successful Xcode Cloud Run 43 `3fabe55c-63fe-4eca-991d-b342da404b73`, with its only action `d055b536-aa03-46fb-9dbe-bc255573db55` successful. The workflow was disabled after the exactly-one trigger. ASC Build 43 `3afbaa31-05a8-4c86-b89f-2e8549fcd73f` is `VALID`, `APP_STORE_ELIGIBLE`, non-expired, and attached to iOS 1.0 `READY_FOR_REVIEW`; this is not `WAITING_FOR_REVIEW`, TestFlight distribution, or public release.
- Review-submission read-back: The independent 2026-08-12 review submission draft `6e901867-779d-454f-8258-bf434993744c` is `READY_FOR_REVIEW`, has `submittedDate: null`, and contains one item including appStoreVersion `583c6199-bf1e-42d9-8bd4-69c2e51f4d2c`. This distinguishes the version/build attachment from the review-submission draft and proves no formal submission occurred.
- Device boundary: Development artifact SHA-256 `46628b8ea4febc9b35d1f4fcdfad7ada5a0fd8c2b4d5ab6fd9c284beb469fc37` was verified as `com.vetmap.app`, `1.0 (43)`, Team `637V678N3Q`, then installed and read back on physical iPhone `是條小狗`. Launch was attempted twice and denied only because the device was locked. No launch, UI, or full functional smoke proof is claimed; the full smoke remains unchecked.
- Backend and public audit: Compatible `FirestoreRules.rules` compiled and released. `functions:purgeUserData` was updated successfully in `asia-east1` as a Node.js 22 v2 callable. An actual unauthenticated POST returned HTTP `401`, status `UNAUTHENTICATED`, and message `You must sign in again before deleting your account.` GitHub Java 21 tests are the successful test gate. The local emulator attempt was stopped as inconclusive after more than five minutes at the first TAP case, and is neither a pass nor a failure. The post-deploy public-only audit passed: 180 clinics (179 authorised + 1 demo), 161 mappable, 18 list-only, availability `11/10/1`, one review, one quote, 124 HK services, three official insurance links, and legacy anonymous reads denied. It cannot prove hidden inventory.
- Remaining submission gates: App Privacy Product Interaction is unpublished because both available browser sessions require ASC login. Content Rights remains `DOES_NOT_USE_THIRD_PARTY_CONTENT`; before formal submission, the account holder must attest to rights covering the clinic database, Hong Kong service directory, official insurance links, and user-submitted content under the terms. The account holder has authorized the send-for-review workflow, so automation may submit only after every listed gate and this legal attestation are complete. Release type remains `MANUAL`; no public release occurred.
- Preserved product scope: Registration, clinic/review/quote submissions, Helpful, reports, blocks, and chat remain open. No claim is made for TestFlight distribution, device launch, UI smoke, Product Interaction publication, Content Rights correction, formal submission, or public release.
